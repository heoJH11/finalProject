	...
	<div class="row">
		<div class="col-lg-12">
			<div class="card px-2">
				<div class="card">
					<div class="card-body">
					<div class="row" style="margin:25px 0 0 50px;">
						<h1  id="oneInqrySj" style="width:1000px; font-size:25px;">${oneInqryVO.oneInqrySj}</h1>
						<p class="card-description" style="font-size:15px;"><fmt:formatDate pattern="yyyy.MM.dd HH:mm" value="${oneInqryVO.oneInqryWritngDt}"/></p>
					</div>	
						<hr />
						<div class="row">
								...
								<div class="tab-content tab-content-custom-pill" id="pills-tabContent-custom">
									...
									<!-- form 시작 -->
										...
										<div>
											...
											<pre style="font-size:20px; margin-bottom:90px; background: transparent;" id="oneInqryCn"><c:out value="${oneInqryVO.oneInqryCn}"/></pre>
										</div>
									</form>
									<!-- form 끝 -->
									<!-- 답글 -->
									<c:choose>
										<c:when test="${oneInqryVO.oneInqryAnswerVOList[0].oneInqryAnswerCn != null}">
											...
											<div class="row" style="align-items: center;">
								            	<label for="oneInqryAnswerCn" class="col-form-label" style="font-size:25px; width:850px;">✨  문의 답글</label>
							    	        	...
							    	        	<div id="oneInqryAnswerCn">
							    	        		<pre style="background: transparent; height: 100%; font-size:18px;" class="form-control"><c:out value="${oneInqryVO.oneInqryAnswerVOList[0].oneInqryAnswerCn}"/></pre>
							    	        	</div>         	
											</div>
										</c:when>
										<c:when test="${oneInqryVO.oneInqryAnswerVOList[0].oneInqryAnswerCn == null && admSession.userId == 'testAdmin'}">
...
$(document).ready(function(){
	var loginId = "${admSession.userId}";
	console.log("loginId", loginId);
	 
	//프로의 문의 답글 등록 버튼
	if(loginId == "testAdmin" && ${oneInqryVO.oneInqryAnswerVOList[0].oneInqryAnswerCn == null} ){
		console.log("관리자다!");
	...
	// 답글 등록 전 회원 문의 수정
	if(${oneInqryVO.oneInqryAnswerVOList[0].oneInqryAnswerCn == null} && loginId != "testAdmin" ){
		console.log("회원이다!");
	...
	$(".clsBtn").on("click",function(){
		console.log("아이디 체크 : " + loginId );
		if(loginId == "testAdmin"){
			location.href="/manage/oneInqryList"
		}else{
			location.href="/oneInqry/oneInqryList"
		}
