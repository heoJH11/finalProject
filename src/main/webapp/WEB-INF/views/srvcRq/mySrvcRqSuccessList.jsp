...
$(function(){
	...
	$.ajax({
		url : "/srvcRequst/mySrvcRqSuccessList",
	    ...
	    success : function(res){
            ...
            if(length == 0){
            	str = "<td colspan='6' style='font-family: GmarketSansLight; font-size:15pt;' >완료된 요청 내역이 없습니다.</td>"
            ...
            $.each(res.content, function(i, v){
                ...
                case "ET02":
                    ...
                    if (v.srvcRequstProcessAt == 1 && v.srvcRequstProcessMber == 0) {
                        str += "<td><button type='button' class='btn btn-secondary btn-sm' id='S_processAtBtn" + v.srvcRequstNo + "' onclick=\"S_processAtBtn('" + v.srvcRequstNo + "')\"> - </button></td>";
                    } else if (v.srvcRequstProcessMber == 1 && v.srvcRequstProcessPro == 1) {
                        str += "<td><button type='button' class='btn btn-success btn-sm' id='S_processSucessBtn" + v.srvcRequstNo + "' disabled='disabled'>이용 완료</button>";
                        ...
                        str += "<td><button type='button' class='btn btn-success btn-sm' id='S_processSucessBtn" + v.srvcRequstNo + "' disabled='disabled'>이용 완료</button></td>";
                    } else {
                        str += "<td></td>";
                    }
                    ...
                    // 한 행 완성
                    str += "</tr>";
                    $("#successTabBody").html(str); // bodyList에 행 추가
                    break;
		...
		$.ajax({
	        ...
	        $.ajax({
	            ...
	            success : function(res){
	                console.log("작성된 리뷰 리스트 res : ", res);
	                $.each(res,function(i, v){
	                    console.log($("#reviewBtn"+v.SRVC_REQUST_NO).attr("value"));
	                    console.log("res 값",v.SRVC_REQUST_NO);
	                    if(v.SRVC_REQUST_NO == $("#reviewBtn"+v.srvcRequstNo).attr("value")){
	                    	$("#reviewBtn"+v.srvcRequstNo).text("리뷰완료");
	                    	$("#reviewBtn"+v.srvcRequstNo).attr("onclick", "location.href='/srvcRqReview/reMgmt'");
	                    }1
	                });
...
// 		window.location.href="/srvcRequst/srvcRqSuccessList";
	});
		...
		success : function(res) {
			...
			if (res.res == 1 && res.vSrvcRequstVO.emplyrTy == "ET01") {
				...
				if(res.vSrvcRequstVO.srvcRequstProcessPro == 1){
					$("#A_processSuccessBtn"+srvcRequstNo).after("<button type='button' class='btn btn-light btn-sm' id='reviewBtn"+srvcRequstNo+"'>리뷰 쓰기</button>");
				}
			}
		}
	});
	...
	$.ajax({
		...
		success : function(res) {
			...
			if (res.res == 1 && res.vSrvcRequstVO.emplyrTy == "ET01") {
				...
				if(res.vSrvcRequstVO.srvcRequstProcessPro == 1){
					$("#Srch_processSuccessBtn"+srvcRequstNo).after("<button type='button' class='btn btn-light btn-sm' id='reviewBtn"+srvcRequstNo+"'>리뷰 쓰기</button>");
				}
			}
		}
	});
...
function S_processAtBtn(no) {
	...
	$.ajax({
		...
		success : function(res) {
			...
			if (res.res == 1 && res.vSrvcRequstVO.emplyrTy == "ET02") {
				console.log("프로가 나와야 되거든요?");
				$("#S_processAtBtn" + srvcRequstNo).text("이용완료");
				$("#S_processAtBtn" + srvcRequstNo).attr("class","btn btn-success btn-sm");
				$("#S_processAtBtn" + srvcRequstNo).attr("id", "S_processSuccessBtn"+srvcRequstNo);
				$("#S_processSuccessBtn"+srvcRequstNo).attr("disabled", true);
			} 
			...
			if (res.res == 1 && res.vSrvcRequstVO.emplyrTy == "ET01") {
				...
				if(res.vSrvcRequstVO.srvcRequstProcessPro == 1){
					$("#S_processSuccessBtn"+srvcRequstNo).after("<button type='button' class='btn btn-light btn-sm' id='reviewBtn"+srvcRequstNo+"'>리뷰 쓰기</button>");
				}
			}
		}
	});
	...
	<div class="card-body">
		<ul class="nav nav-tabs" role="tablist">
			...
			<li class="nav-item" role="presentation"><a class="nav-link active" style="font-family: 'GmarketSansLight';"
				...
				aria-controls="success-1" aria-selected="true" tabindex="-1">요청 수락</a>
			</li>
			<li class="nav-item" role="presentation"><a class="nav-link" style="font-family: 'GmarketSansLight';"
		...
		<div class="tab-content">
			...
			<!-- 완료 탭 -->
			<div class="tab-pane fade active show" id="successTab" role="tabpanel"
